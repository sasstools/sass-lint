...
describe('config', function () {
  it('should return the defaults if no config is passed in', function (done) {
    var defaultConfig = yaml.load(fs.readFileSync(path.join(__dirname, '..', 'lib', 'config', 'sass-lint.yml'), 'utf8')),
        conf = config();
  ...
  it('should use a config file if one is provided', function (done) {
    var defaultConfig = yaml.load(fs.readFileSync(path.join(__dirname, '..', 'lib', 'config', 'sass-lint.yml'), 'utf8')),
        ...
        conf = config(tempOptions),
        merged = merge.recursive(defaultConfig, tempOptions, yaml.load(fs.readFileSync(path.join(__dirname, 'yml', '.stylish-output.yml'), 'utf8')));
  ...
  it('should not merge default rules when `merge-default-rules` is false in config file [check equal]', function (done) {
    var customConfig = yaml.load(fs.readFileSync(path.join(__dirname, 'yml', '.no-merge-default.yml'), 'utf8')),
        tempOptions = custOptions(),
        conf,
        merged;
