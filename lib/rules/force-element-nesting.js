...
var formatOutput = function (str) {
  str = helpers.camelCaseToHyphens(str);
  str = helpers.capitalize(str);
  return str;
};
        ...
        selector.forEach('simpleSelector', function (simpleSelector) {
          ...
          simpleSelector.forEach(function (node) {
            ...
            var constructedSelector = helpers.constructSelector(node),
                currentVal = constructedSelector.currentValue;

            if (helpers.isNestable(currentVal, previousVal, elements, nestableElements)) {
              helpers.addUnique(result, {
                'ruleId': parser.rule.name,
                'line': node.start.line,
                'column': node.start.column,
                'message': formatOutput(constructedSelector.currentValue) + ' `' + constructedSelector.content + '` should be nested within its parent ' + previousVal,
