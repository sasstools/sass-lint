...
module.exports = {
  ...
  'detect': function (ast, parser) {
    ...
    if (parser.options.allowRgba && colorFunctions.indexOf('rgba') !== -1) {
      colorFunctions.splice(colorFunctions.indexOf('rgba'), 1);
    }
    ...
    ast.traverseByType('value', function (node, i, parent) {
      node.forEach(function (valElem) {
        ...
        // if not a color value or a variable then check if it's a function
          ...
          else if (parser.options.allowRgba && funcType === 'rgba' && valElem.content[1].content[0].type !== 'variable' && declarationType !== 'variable' ) {
            result = helpers.addUnique(result, {
              'ruleId': parser.rule.name,
              'line': valElem.start.line,
              'column': valElem.start.column,
                  ...
                  // if the argument is a function itself
                    ...
                    funcArgs.forEach( function (nestedFuncArgs) {
                      ...
                      if (
                        ...
                        (cssColors.indexOf(nestedFuncArgs.content) !== -1 || helpers.isValidHex(nestedFuncArgs.content) || colorFunctions.indexOf(nestedFuncArgs.content) !== -1 )
                      ) {
                        result = helpers.addUnique(result, {
                          'ruleId': parser.rule.name,
