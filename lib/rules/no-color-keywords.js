...
module.exports = {
  ...
  'defaults': {},
  'detect': function(ast, parser, roots) {
    var result = [];
        ...
        if (elem.type === 'ident' && !isVariable) {
          ...
          if (index !== -1) {
            result = helpers.addUnique(result, {
              ...
              'fix': (ast) => {
                ast.traverseByType('value', (n, i, p) => {
                  n.traverseByTypes('ident', (_n, _i, _p) => {
                    if (!_p.is('variable')){
                      var index = cssColors.indexOf(_n.content.toLowerCase());
                      if(index !== -1)
                        _n.content = _n.content.replace(new RegExp(`(${cssColors.join('|')})`), `#${cssColors[index + 1]}`);
                    }
                  });
                });
              }
