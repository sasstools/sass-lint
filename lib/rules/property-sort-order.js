...
var sortAlphabetical = function(prev, curr) {
...
module.exports = {
  ...
  'fix': function (groot, parser) {
    groot.tree.traverseByType('block', function (block) {
      var res = [];
      var indexes = [];
      block.forEach('declaration', function(declaration, index){
        ...
        if (name) {
          ...
          console.log(declaration.first('property'));
          debugger;
        }
      });
      if(parser.options.order === 'alphabetical'){
        res.sort(sortAlphabetical);
      }
