...
var isConditionOnNewLine = function (node, parentNode, j) {
  ...
  if (node.contains('ident') && node.first('ident').content === 'else') {
    ...
    let previousChild = parentNode.get(--j);
...
module.exports = {
  ...
  'detect': function (ast, parser) {
    ...
    ast.traverseByTypes(['conditionalStatement', 'atrule', 'ruleset', 'mixin', 'loop'], function (block, i, parent) {
      ...
      /**
       * Store nodes that we will use to run rules against
       */
      ...
      if (block.is('conditionalStatement')) {
        let previousParent = block.contains('condition') ? block.last('condition') : false;
        previousNode = previousParent.contains('atkeyword') ? previousParent.last('atkeyword') : false;
      }
      ...
      /**
       * Node checks - If we've picked up a return @rule ignore it
       */
      if (block.is('atrule')) {
      ...
      rules.singleLineStatement = isSingleLineStatement(block);

      // Determine if condition is on a new line
      if (block.is('atrule') || block.is('conditionalStatement')) {
        rules.conditionOnNewLine = isConditionOnNewLine(previousNode, parent, i);
      }

      // Determine if opening brace is on new line
      if (previousNode && currentNode) {
        rules.openingBraceOnNewLine = isOpeningBraceOnNewLine(previousNode, currentNode);
      }

      // Determine if closing brace is on new line
      rules.closingBraceOnNewLine = isClosingBraceOnNewLine(block);

      /**
       * Build results
       */

      if (rules.singleLineStatement === false && rules.closingBraceOnNewLine === false) {
      ...
      if (rules.singleLineStatement === true) {
      ...
      if (previousNode && currentNode) {
        ...
        if (parser.options.style === '1tbs') {
          if (rules.openingBraceOnNewLine === false) {
            result = helpers.addUnique(result, {
              'ruleId': parser.rule.name,
          ...
          if (rules.conditionOnNewLine === true) {
            result = helpers.addUnique(result, {
              'ruleId': parser.rule.name,
        ...
        if (parser.options.style === 'stroustrup') {
          ...
          if (rules.conditionOnNewLine === false) {
            result = helpers.addUnique(result, {
              'ruleId': parser.rule.name,
              'line': previousNode.start.line,
              'column': previousNode.start.column,
        ...
        if (parser.options.style === 'allman') {
          if (rules.openingBraceOnNewLine === true) {
            result = helpers.addUnique(result, {
              'ruleId': parser.rule.name,
              'line': currentNode.start.line,
              'column': currentNode.start.column,
