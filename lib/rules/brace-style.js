 ...
 * @returns {bool} Wether or not the it is an exception
 */
 ...
 * @param {int} i - The index of the original node
 * @returns {Object} The results of the rule checks
 */
var runRuleChecks = function (node, currentNode, previousNode, parentNode, i) {
  var checks = {};
  ...
  if (node.is('atrule') || node.is('conditionalStatement')) {
    checks.conditionOnNewLine = isConditionOnNewLine(previousNode, parentNode, i);
  }
